...
Location: `.SYSTEM/PROJECT-SPECIFIC/mcp/mcp.json`
...
```bash
# Node.js tools
npm install -g @executeautomation/playwright-mcp-server @modelcontextprotocol/server-sequential-thinking
```

**Permission errors**
```bash
# Use sudo for global npm (Linux/Mac)
sudo npm install -g @executeautomation/playwright-mcp-server @modelcontextprotocol/server-sequential-thinking
```
...
1. **Set environment variables** for API keys and connections
2. **Test tools individually** before using in workflows
3. **Document decisions** using sequential-thinking
4. **Store knowledge** in postgres for future reference
5. **Use web search** for research and problem solving
6. **Integrate with GitHub** for documentation and issues
...
3. Check installation status: `python .SYSTEM/setup-mcp-system.py --test`
4. Run comprehensive tests: `python .SYSTEM/test-system/test-universal-system.py`
