...
def test_status_endpoints():
    ...
    try:
        ...
        assert response.status_code == 200, f"System status endpoint failed: {response.status_code}"
        ...
        pytest.fail(f"System status endpoint error: {e}")
    ...
    try:
        ...
        pytest.fail(f"API status endpoint error: {e}")
    ...
    try:
        ...
        pytest.fail(f"Connect endpoint error: {e}")
    
    # Test 5: Queue status endpoint
    try:
        response = requests.get(f"{base_url}/api/queue/status", timeout=5)
        assert response.status_code == 200, f"Queue status endpoint failed: {response.status_code}"
        ...
        pytest.fail(f"Queue status endpoint error: {e}")

@pytest.mark.skip(reason="Requires running Forge server")
def test_forge_connection():
    ...
    try:
        ...
        assert response.status_code == 200, f"Forge API returned status {response.status_code}"
        ...
        pytest.skip(f"Forge API not accessible: {e}")
