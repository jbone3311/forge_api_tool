...
class TestIntegration(unittest.TestCase):
    ...
    def test_output_manager_integration(self):
        """Test integration between output manager and other components."""
        # Create test outputs
        ...
        for i, prompt in enumerate(prompts):
            ...
            # Save image
            filepath = self.output_manager.save_image(image_data, config_name, prompt, i)
            
            # Verify file was created
            self.assertTrue(os.path.exists(filepath))
        
        # Get outputs for the config
        outputs = self.output_manager.get_outputs_for_config(config_name)
        
        # Should have 3 outputs
        self.assertEqual(len(outputs), 3)
        
        # Check that all outputs have required fields
